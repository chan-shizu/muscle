...
%                         muscleFiberF(i,s)=HillActive(i,s-ul(1,2))+HillPassive(i,s-ul(1,2));%受動的な力と能動的な力の和
            ...
            if powerFlag == 0
                ...
                for k=ul(2)+1:ul(3)
                    muscleFiberF_x(i,se(k,3)) = -muscleFiberFInput(i,k)*cos(se(k,4))*cos(se(k,5));
                    muscleFiberF_y(i,se(k,3)) = -muscleFiberFInput(i,k)*cos(se(k,4))*sin(se(k,5));
                    muscleFiberF_z(i,se(k,3)) = -muscleFiberFInput(i,k)*sin(se(k,4));
                end
            end
            
            FsSum_x(i,:)=Fs_x(i,:,1)+Fs_x(i,:,2)+Fs_x(i,:,3)+Fs_x(i,:,4)+Fs_x(i,:,5)+Fs_x(i,:,6);%+muscleFiberF_x(i,:);
            FsSum_y(i,:)=Fs_y(i,:,1)+Fs_y(i,:,2)+Fs_y(i,:,3)+Fs_y(i,:,4)+Fs_y(i,:,5)+Fs_y(i,:,6);%+muscleFiberF_y(i,:);
            FsSum_z(i,:)=Fs_z(i,:,1)+Fs_z(i,:,2)+Fs_z(i,:,3)+Fs_z(i,:,4)+Fs_z(i,:,5)+Fs_z(i,:,6);%+muscleFiberF_z(i,:);
