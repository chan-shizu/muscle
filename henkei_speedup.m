...
data_displacement_plane = csvread(pathDataDisplacementPlane, 0, 0)*(10^17);%/1000;
data0=csvread(pathDataInitial, 0, 0)*(10^17);%/1000;
% data_displacement_plane= vpa(data_displacement_plane,20);
% data0=vpa(data0,20);
% data_displacement_plane= double(data_displacement_plane);
% data0=double(data0);
se=csvread(file_name_se, 0, 0);
tetra=csvread(file_name_tetra, 0, 0);

timeNum = size(data_displacement_plane);%時間ステップ
pointNum = size(data0);
seNum = size(se);
tetraNum = size(tetra);
dt=1*10^(-3);
...
for i=1:timeNum(1)%iの値は時間ステップ
    i
    %プログラムの高速化のための事前割り当て
%     matchA = zeros(tetraNum(2)*tetraNum(1)/5,1);
%     matchB = matchA;
%     matchC = matchA;
%     matchD = matchA;
    ...
    if i==1
        ...
        %強制変位を与える面
        for k=1:y*t
            datai_x(i,(pointNum(1)-k+1))=data_displacement_plane(i,3*(y*t-k)+1);  %x座標
            datai_y(i,(pointNum(1)-k+1))=data_displacement_plane(i,3*(y*t-k)+2);
            datai_z(i,(pointNum(1)-k+1))=data_displacement_plane(i,3*(y*t-k)+3);
        end
    end
...
%     match_r = match_l;
    ...
    for j=1:tetraNum(1)/5;
        for k=1:tetraNum(2);
            ...
            %体積時系列
            if data_displacement_plane(i,3)-data_displacement_plane(1,3)>0;    %こっちが標準
