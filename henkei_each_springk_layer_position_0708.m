    ...
    for s=1:seNum(1)
        ...
        if se(s,1)==2  %筋線維だけHillのモデル計算
            ...
            if i == 1
                ...
                Vce(i,s-ul(1,2)) = -(length_s(i,s) - length_s(i-1,s))/(10^-5);%0.1ではなくdtでは?        
%                 Vce(i,s-ul(1,2)) = -(length_s(i,s) - length_s(i-1,s))/dt;
            ...
            HillActive(i,s-ul(1,2)) = alpha .* f_Lce(i,s-ul(1,2)) .* f_Vce(i,s-ul(1,2)) .* Fmax'; %占部さんの修論，式(3-29)
            if i==1
                springF(i,s)=0;
            else
    ...
    for j=1:tetraNum(1)/5
        for k=1:tetraNum(2)
            ...
            if data(i,3)-data(1,3)>0    %こっちが標準,それぞれの四面体ごとに体積保存係数を設定
                ...
                fvB(:,k+(j-1)*tetraNum(2))=1/6*springkVPk(2,k+(j-1)*tetraNum(2))*(V(k+(j-1)*tetraNum(2),1)-V0(k+(j-1)*tetraNum(2),1))/V0(k+(j-1)*tetraNum(2),1)^2*cross(Vec1(:,2),Vec2(:,2));
                fvC(:,k+(j-1)*tetraNum(2))=1/6*springkVPk(2,k+(j-1)*tetraNum(2))*(V(k+(j-1)*tetraNum(2),1)-V0(k+(j-1)*tetraNum(2),1))/V0(k+(j-1)*tetraNum(2),1)^2*cross(Vec2(:,3),Vec1(:,3));
                fvD(:,k+(j-1)*tetraNum(2))=1/6*springkVPk(2,k+(j-1)*tetraNum(2))*(V(k+(j-1)*tetraNum(2),1)-V0(k+(j-1)*tetraNum(2),1))/V0(k+(j-1)*tetraNum(2),1)^2*cross(Vec1(:,4),Vec2(:,4));
            else
                fvA(:,k+(j-1)*tetraNum(2))=1/6*springkVPk(2,k+(j-1)*tetraNum(2))*(V(k+(j-1)*tetraNum(2),1)-V0(k+(j-1)*tetraNum(2),1))/V0(k+(j-1)*tetraNum(2),1)^2*cross(Vec1(:,1),Vec2(:,1));
                fvB(:,k+(j-1)*tetraNum(2))=1/6*springkVPk(2,k+(j-1)*tetraNum(2))*(V(k+(j-1)*tetraNum(2),1)-V0(k+(j-1)*tetraNum(2),1))/V0(k+(j-1)*tetraNum(2),1)^2*cross(Vec2(:,2),Vec1(:,2));
                fvC(:,k+(j-1)*tetraNum(2))=1/6*springkVPk(2,k+(j-1)*tetraNum(2))*(V(k+(j-1)*tetraNum(2),1)-V0(k+(j-1)*tetraNum(2),1))/V0(k+(j-1)*tetraNum(2),1)^2*cross(Vec1(:,3),Vec2(:,3));
                fvD(:,k+(j-1)*tetraNum(2))=1/6*springkVPk(2,k+(j-1)*tetraNum(2))*(V(k+(j-1)*tetraNum(2),1)-V0(k+(j-1)*tetraNum(2),1))/V0(k+(j-1)*tetraNum(2),1)^2*cross(Vec2(:,4),Vec1(:,4));
            end
        end
    end
