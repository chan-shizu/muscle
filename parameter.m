...
data0=csvread(file_name_data0, 0, 0); %mmからmに変換
...
%         Y(12*(j-1)+1:12*j,1,i)=KCST(j,1:12,i);
%         X(12*(j-1)+1:12*j,1,i)=KMSM(j,1:12,i);   %k
% %         X(12*(j-1)+1:12*j,1,i)=KC(j,1:12,i);   %ks
%     end
%     a(1,:,i)=X(:,:,i)\Y(:,:,i);
% %     a(:,:,i)=lsqminnorm(X(:,:,i),Y(:,:,i));
% end
...
%         end
% end
% settingAllay = [100 100 100 100 100 100];
% for n=1:6
%     if n==1
...
% plot(V0);
springkAllaySum(1,:) = sum(springkAllay');% ./ (seCount-1);
springkVPk(1,1:seNum(1)) = springkAllaySum;%x20
% springkVPk(1,1:seNum(1)) = ;%mean(springkVPk(1,:)); 
% springkVPk(2,1:tetraNum(1)/5*tetraNum(2)) = mean(springkVPk(2,:));
% springkVPk(2,1:tetraNum(1)/5*tetraNum(2)) =0;%mean(springkVPk(2,:));
