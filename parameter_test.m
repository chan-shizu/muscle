...
% springkVPk(1,1:seNum(1)) = springAllayAve;
% sizeul = size(ul)
% 
% for n=1:sizeul(2)
%     if n==1
%         springkVPk(1,1:ul(n)) = mean(springkVPk(1,1:ul(n)));
%     else
%         springkVPk(1,ul(n-1)+1:ul(n)) = mean(springkVPk(1,ul(n-1)+1:ul(n)));
%     end
% end
...
% springkVPk(1,1:ul(2)) = springkVPk(1,1)*2
